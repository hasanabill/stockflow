...
export async function GET(request: Request) {
    ...
    const deliveredSales = await Sale.aggregate([
        { $match: { business: new (require("mongoose").Types.ObjectId)(businessId), status: "delivered", createdAt: { $gte: fromDate, $lte: toDate } } },
    ...
    const saleLedger = await InventoryLedger.aggregate([
        { $match: { business: new (require("mongoose").Types.ObjectId)(businessId), sourceType: "SALE", createdAt: { $gte: fromDate, $lte: toDate } } },
    ...
    const expensesAgg = await Expense.aggregate([
        { $match: { business: new (require("mongoose").Types.ObjectId)(businessId), date: { $gte: fromDate, $lte: toDate } } },
